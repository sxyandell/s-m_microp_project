...
// Switch mapping (active-low): 1..8 -> PA12, PB0, PB7, PB6, PB1, PC14, PC15, PA8
static const int switch_pins[8] = { PA12, PB0, PB7, PB6, PB1, PC14, PC15, PA8 };
...
static void enable_pullups_for_switches(void) {
	...
	GPIOB->PUPDR &= ~(0x3u << (2 * 1));
	GPIOB->PUPDR |=  (0x1u << (2 * 1));  // pull-up
	GPIOB->PUPDR &= ~(0x3u << (2 * 6));
	GPIOB->PUPDR |=  (0x1u << (2 * 6));  // pull-up
	GPIOB->PUPDR &= ~(0x3u << (2 * 7));
	GPIOB->PUPDR |=  (0x1u << (2 * 7));  // pull-up
	// PC14, PC15
	GPIOC->PUPDR &= ~(0x3u << (2 * 14));
	GPIOC->PUPDR |=  (0x1u << (2 * 14)); // pull-up
	GPIOC->PUPDR &= ~(0x3u << (2 * 15));
	GPIOC->PUPDR |=  (0x1u << (2 * 15)); // pull-up
}
